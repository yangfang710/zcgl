...
function tigra_tables (str_tableid, // table id (req.)
  ...
  init_color=(str_onclick_color ? str_onclick_color : '#4C7DAB');
...
function init_tr(tr_id){
  var row = document.getElementById(tr_id);
  eval(tr_id+".style.backgroundColor='"+init_color+"'");
  //row.set_color(row.config.onclick_color);
  row.lconfig.clicked = row;
//TR<%=first%>.style.backgroundColor="#E4FAB5";
}
function mOvr(src) {
